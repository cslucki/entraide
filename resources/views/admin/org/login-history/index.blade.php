<x-org-admin-layout title="Historique de connexion" :organization="$organization">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Historique de connexion</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Connexions réussies des utilisateurs de {{ $organization->name }}</p>
    </div>
    ...
    <form method="GET" class="mb-5 flex flex-wrap gap-3">
        ...
        <a href="{{ route('organization.admin.stats.login-history', $organization) }}" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-600 dark:text-gray-400">Effacer</a>
        @endif
    </form>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <table class="w-full text-sm">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                        <a href="{{ route('organization.admin.stats.login-history', array_merge(request()->query(), [$organization, 'sort' => 'user', 'direction' => request('sort') === 'user' && request('direction') === 'asc' ? 'desc' : 'asc'])) }}" class="hover:text-indigo-600 dark:hover:text-indigo-400">
                            Utilisateur
                            @if(request('sort') === 'user') <span>{{ request('direction') === 'asc' ? '↑' : '↓' }}</span>@endif
                        </a>
                    </th>
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                        <a href="{{ route('organization.admin.stats.login-history', array_merge(request()->query(), [$organization, 'sort' => 'ip_address', 'direction' => request('sort') === 'ip_address' && request('direction') === 'asc' ? 'desc' : 'asc'])) }}" class="hover:text-indigo-600 dark:hover:text-indigo-400">
                            IP
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                        <a href="{{ route('organization.admin.stats.login-history', array_merge(request()->query(), [$organization, 'sort' => 'created_at', 'direction' => request('sort') === 'created_at' && request('direction') === 'asc' ? 'desc' : 'asc'])) }}" class="hover:text-indigo-600 dark:hover:text-indigo-400">
                            Date
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <a href="{{ route('organization.admin.stats.login-history.user', [$organization, $log->user]) }}" class="font-medium text-gray-900 dark:text-gray-100 hover:text-indigo-600 truncate">
                                    {{ $log->user->name }}
                                </a>
                ...
                @empty
                <tr>
                    <td colspan="4" class="px-4 py-8 text-center text-sm text-gray-400">Aucune connexion enregistrée.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
