<x-org-admin-layout :title="__('crm.email.title', ['name' => $contact->fullName])" :organization="$organization">
    ...
    <div class="mb-4">
        <a href="{{ route('organization.admin.crm.contacts.show', ['organization' => $organization->slug, 'contact' => $contact->id]) }}" class="text-sm text-indigo-600 dark:text-indigo-400 hover:underline" data-crm-email-back>&larr; {{ $contact->fullName }}</a>
    </div>
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
            ...
            <form method="POST" action="{{ route('organization.admin.crm.contacts.email.send', ['organization' => $organization->slug, 'contact' => $contact->id, 'template' => $template->id]) }}" data-crm-email-form>
                @csrf
