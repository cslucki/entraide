    ...
    <div class="max-w-xl space-y-6">
        ...
        @if($canEditCredential)
            @include('admin.org._ai-credential-form', [
                ...
                'hasKey' => $hasKey,
                'cancelUrl' => route('organization.admin.dashboard', $organization),
            ])
        @else
            {{-- TASK-1306 : `platform_managed` — l'admin d'Organization ne voit
                 jamais le formulaire, ni le champ api_key. Seul un statut. --}}
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6" data-ai-credential-managed-by="platform">
                <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide mb-2">{{ __('admin.organization_ai_platform_managed_title') }}</h3>
                <p class="text-sm {{ $hasKey ? 'text-green-700 dark:text-green-300' : 'text-amber-700 dark:text-amber-300' }}">
                    {{ $hasKey ? __('admin.organization_ai_platform_managed_configured') : __('admin.organization_ai_platform_managed_not_configured') }}
                </p>
            </div>
        @endif

        @if($isSuperAdmin)
            {{-- TASK-1306 : qui gere ce credential — reserve au SuperAdmin, la
                 vue ne fait qu'omettre le controle pour l'admin d'Organization ;
                 la garde reelle est cote serveur (updateAiCredentialMode()). --}}
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                @include('admin.org._ai-credential-mode-form', ['organization' => $organization, 'credentialMode' => $credentialMode])
            </div>
        @endif
