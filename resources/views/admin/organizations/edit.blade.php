<x-admin-layout title="Éditer l'organisation">
    <div class="max-w-2xl">
        <form method="POST" action="{{ route('admin.organizations.update', $organization) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
                <h2 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Publication des annonces</h2>
                <p class="text-xs text-gray-500 dark:text-gray-400">Qui peut publier des annonces dans le Flux de cette organisation ?</p>
                <div class="space-y-2">
                    <label class="flex items-center gap-3 cursor-pointer">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400">La langue sélectionnée s'applique à l'interface pour tous les membres de cette organisation.</p>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Langue d'affichage</label>
                    <select name="locale"
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-sm">
                        <option value="fr" {{ old('locale', $organization->locale ?? 'fr') === 'fr' ? 'selected' : '' }}>Français</option>
                        <option value="en" {{ old('locale', $organization->locale ?? 'fr') === 'en' ? 'selected' : '' }}>English</option>
                    </select>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Les membres verront l'interface dans cette langue. Ils peuvent toujours utiliser les URLs publiques.</p>
                </div>
            </div>
