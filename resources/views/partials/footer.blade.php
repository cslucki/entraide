    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div class="flex flex-col sm:flex-row items-center justify-center gap-2 text-xs text-gray-400 dark:text-gray-500">
            <div class="relative flex flex-wrap items-center justify-center text-center divide-x divide-gray-300 dark:divide-gray-700">
                ...
                {{-- TASK-1612 / TASK-1613 — « Open Source » n'emmene plus
                     hors de BouclePro : il mene a la page qui donne a voir
                     la racine du depot public.

                     Une PAGE, pas une surcouche : TASK-1612 avait fait une
                     surcouche, qui faisait porter son CSS et son JS a toutes
                     les pages du produit et n'existait pas sur les gabarits
                     autonomes (dont la homepage). Arbitrage Cyril : ici il
                     n'y a plus qu'un lien.

                     `organizationScopedUrl()`, comme les mentions legales :
                     sous `/org/{slug}` la page est servie DANS le contexte
                     de l'Organization, ailleurs elle est globale. Le
                     declencheur est le prefixe de l'URL, jamais un tenant
                     devine par defaut (borne TASK-1602/1604). MEME vue dans
                     les deux cas : rien n'est duplique.

                     Un lien interne, donc ni `target` ni `rel` : on reste
                     dans BouclePro. --}}
                <a href="{{ organizationScopedUrl('open-source.page', 'organization.open-source') }}"
