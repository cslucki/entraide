    ...
    <div class="max-w-3xl mx-auto px-4 py-8">
        ...
        <form method="POST" action="{{ $_reqUpdateAction }}" enctype="multipart/form-data" data-marketplace-validation
            ...
            <div class="mb-5 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl text-sm text-amber-800 dark:text-amber-200">
                ...
                <p class="opacity-90">{{ __('marketplace.request_points_hint') }}</p>
            </div>

            <div class="mb-5 grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('requests.edit.budget_min') }}</label>
                    <input type="number" name="budget_min" value="{{ old('budget_min', $request->budget_min) }}" @if($pointMin !== null) min="{{ $pointMin }}" @endif @if($pointMax !== null) max="{{ $pointMax }}" @endif required
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    ...
                    <input type="number" name="budget_max" value="{{ old('budget_max', $request->budget_max) }}" @if($pointMin !== null) min="{{ $pointMin }}" @endif @if($pointMax !== null) max="{{ $pointMax }}" @endif
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500">
                </div>
            </div>
