...
<div>
    <x-slot:title>{{ __('feed-post.my_posts') }}</x-slot:title>

    <div class="max-w-4xl mx-auto px-4 py-8">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ __('feed-post.my_posts') }}</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ __('feed-post.my_posts_subtitle') }}</p>
            </div>
            @if($canCreate)
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
                    <div class="flex items-start justify-between gap-4">
                        <div class="min-w-0 flex-1">
                            <h3 class="font-medium text-gray-900 dark:text-gray-100">{{ $post->title ?: __('feed-post.untitled') }}</h3>
                            <p class="text-sm text-gray-500 mt-1">{{ Str::limit(strip_tags($post->content), 200) }}</p>
                            <p class="text-xs text-gray-400 mt-2">{{ __('feed-post.created_at', ['date' => $post->created_at->setTimezone('Europe/Paris')->isoFormat('lll')]) }}</p>
                            @if($post->scheduled_at)
                                <p class="text-xs text-gray-400">{{ __('feed-post.scheduled_at', ['date' => $post->scheduled_at->setTimezone('Europe/Paris')->isoFormat('lll')]) }}</p>
                            @endif
                            @if($post->published_at)
                                <p class="text-xs text-gray-400">{{ __('feed-post.published_at', ['date' => $post->published_at->setTimezone('Europe/Paris')->isoFormat('lll')]) }}</p>
                            @endif
                        </div>
                        <div class="flex items-center gap-2 flex-shrink-0">
                            @php
                                ...
                                $badgeLabel = match ($post->status) {
                                    'published' => __('feed-post.status_published'),
                                    'scheduled' => __('feed-post.status_scheduled'),
                                    default => __('feed-post.status_draft'),
                                };
                            @endphp
                            ...
                            @can('pin', App\Models\FeedPost::class)
                                <button type="button" wire:click="togglePin('{{ $post->id }}')"
                                    ...
                                    {{ $post->isPinned() ? __('feed-post.pinned') : __('feed-post.pin_action') }}
                                </button>
                            @endcan
                            <div class="flex items-center gap-1">
                                @if($post->status === 'scheduled')
                                    <button type="button" wire:click="publishNow('{{ $post->id }}')"
                                            class="text-xs text-green-600 hover:underline">{{ __('feed-post.publish_now_action') }}</button>
                                @endif
                                <a href="{{ route($editRouteName, $editRouteParams($post->id)) }}" class="text-xs text-indigo-600 hover:underline">{{ __('feed-post.edit_action') }}</a>
                                <button type="button"
                                        x-on:click="if (confirm('{{ __('feed-post.delete_confirm') }}')) { $wire.delete('{{ $post->id }}') }"
                                        class="text-xs text-red-600 hover:underline">{{ __('feed-post.delete_action') }}</button>
                            </div>
                            <a href="{{ route($showRouteName, $showRouteParams($post->id)) }}" class="text-xs text-indigo-600 hover:underline">{{ __('feed-post.view') }}</a>
                        </div>
                    </div>
            </div>
            @endforeach
