...
<div class="mx-auto max-w-3xl px-4 py-6 sm:px-6 lg:px-8">
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <p class="text-xs font-semibold uppercase tracking-[0.2em] text-indigo-500">{{ __('feed.organization') }}</p>
            <h1 class="mt-1 text-2xl font-bold text-gray-900 dark:text-gray-100">{{ __('feed.title') }}</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('feed.subtitle') }}</p>
    ...
    @if($pinned->isNotEmpty())
        <div class="mb-6 space-y-4">
            <h2 class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('feed.pinned') }}</h2>
    ...
    <div class="space-y-4">
        @if($pinned->isEmpty() && $items->isEmpty())
            <div class="text-center py-12">
                ...
                <p class="text-gray-500 dark:text-gray-400">{{ __('feed.empty') }}</p>
                @can('create', App\Models\FeedPost::class)
