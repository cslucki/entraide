    ...
    <div class="space-y-4 p-4 sm:p-5">
        <header class="flex items-start justify-between gap-3">
            <div class="min-w-0">
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    {{ $post->user?->name ?? 'Membre' }} · {{ $post->published_at?->isoFormat('D MMM YYYY à HH:mm') ?? $post->created_at->isoFormat('D MMM YYYY à HH:mm') }}
                </p>
            </div>
        </header>
        ...
        @if($lastComment)
            <div class="flex items-start gap-2 text-sm">
                <span class="font-semibold text-gray-800 dark:text-gray-100 shrink-0">{{ $lastComment->user?->name ?? 'Membre' }}</span>
        ...
        @if($commentCount > 1)
            <x-modal name="comments-{{ $post->id }}" maxWidth="lg">
                <div class="p-6 space-y-4">
                    <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">Commentaires ({{ $commentCount }})</h3>
                    @foreach($post->comments->sortBy('created_at') as $comment)
                        <div class="border-b border-gray-100 pb-3 last:border-0 dark:border-gray-700">
                            ...
                            <p class="mt-0.5 text-xs text-gray-400">{{ $comment->created_at->isoFormat('D MMM YYYY à HH:mm') }}</p>
                        </div>
                    @endforeach
                </div>
            </x-modal>
