    ...
    @if($footer)
        <div class="mt-2 flex items-center justify-between gap-2 pl-7">
            ...
            <div class="flex items-center gap-1.5">
                ...
                {{-- Actions menu (bottom-right, next to avatars) --}}
                @if($canManage)
                    <div x-data="{ open: false }" class="relative" x-on:click.stop>
                        <button type="button" x-on:click="open = !open" x-bind:aria-expanded="open" aria-haspopup="true"
                                class="flex h-6 w-6 items-center justify-center rounded-lg text-gray-300 transition hover:bg-gray-100 hover:text-gray-600 dark:text-gray-600 dark:hover:bg-gray-800 dark:hover:text-gray-300"
                        ...
                        <div x-show="open" x-cloak x-on:click.outside="open = false" x-transition.opacity
                             class="absolute bottom-full right-0 z-30 mb-1 w-52 overflow-hidden rounded-xl border border-gray-200 bg-white py-1 shadow-lg dark:border-gray-700 dark:bg-gray-800" role="menu">
                            <button type="button" wire:click="openDetail('{{ $item->id }}')" x-on:click="open = false" role="menuitem"
                            ...
                            <button type="button" wire:click="moveUp('{{ $item->id }}')" x-on:click="open = false" role="menuitem" class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs font-medium text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-700/60">{{ __('loops.roadmap_move_up') }}</button>
                            <button type="button" wire:click="moveDown('{{ $item->id }}')" x-on:click="open = false" role="menuitem" class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs font-medium text-gray-700 hover:bg-gray-50 dark:text-gray-200 dark:hover:bg-gray-700/60">{{ __('loops.roadmap_move_down') }}</button>
                            @if($mine)
                                <button type="button" role="menuitem"
                                        ...
                                        class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs font-medium text-red-600 transition hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20">{{ __('loops.roadmap_delete') }}</button>
                            @endif
                        </div>
                    </div>
                @endif
