...
<div data-multi-ai-states role="status" aria-live="polite" aria-atomic="false" class="flex flex-col gap-2">
    ...
    <div wire:loading wire:target="askAssistant,askAllAssistants,retryAssistant,synthesiseAssistants"
        ...
        <span>{{ __('loops.plugins_multi_ai_pending', ['assistant' => __('loops.plugins.multi_ai_assistants.label')]) }}</span>
    </div>
    ...
    @foreach($states as $key => $state)
        ...
        <div data-multi-ai-state="{{ $key }}"
            ...
            <div class="mt-2 flex flex-wrap items-center gap-2">
                ...
                {{-- « demander a une autre IA » : les pairs encore actifs. --}}
                @foreach($assistants as $autre)
                    @continue($autre['key'] === $key)
                    <button type="button"
                            wire:click="askAssistant('{{ $autre['key'] }}')"
                            wire:loading.attr="disabled"
                            wire:target="askAssistant,askAllAssistants,retryAssistant,synthesiseAssistants"
                            data-multi-ai-ask-other="{{ $autre['key'] }}"
                            class="inline-flex items-center gap-1.5 rounded-full border border-amber-200 px-2.5 py-1 font-medium text-amber-800 transition hover:bg-white/70 disabled:cursor-not-allowed disabled:opacity-50 dark:border-amber-700 dark:text-amber-100">
                        {{ __('loops.plugins_multi_ai_ask_one', ['assistant' => $autre['label']]) }}
                    </button>
                @endforeach
