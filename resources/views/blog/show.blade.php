        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <article class="lg:col-span-3">
                ...
                <div class="flex items-center gap-3 mb-6">
                    ...
                    @if($post->status === 'published')
                    <button type="button" x-data="{ openInvite: false }" @click="openInvite = true"
                        class="inline-flex items-center gap-1.5 px-4 py-1.5 border border-indigo-300 dark:border-indigo-600 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 text-sm font-medium rounded-lg transition">
                        ...
                        {{ __('blog.action_invite') }}
                    </button>
                    <div x-show="openInvite" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @keydown.escape.window="openInvite = false">
                        <div @click.away="openInvite = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-md w-full mx-4 p-6">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">{{ __('blog-invitation.modal_title') }}</h3>
                            <form action="{{ $_blogRoute('invite.store', ['post' => $post]) }}" method="POST" x-data="{ sending: false }" @submit="sending = true">
                                @csrf
                                <div class="space-y-3">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('blog-invitation.modal_recipient_email') }} *</label>
                                        <input type="email" name="recipient_email" required placeholder="{{ __('blog-invitation.modal_placeholder_email') }}" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 text-sm">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('blog-invitation.modal_recipient_name') }}</label>
                                        <input type="text" name="recipient_name" placeholder="{{ __('blog-invitation.modal_placeholder_name') }}" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 text-sm">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('blog-invitation.modal_message') }}</label>
                                        <textarea name="message" rows="3" placeholder="{{ __('blog-invitation.modal_placeholder_message') }}" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 text-sm"></textarea>
                                    </div>
                                </div>
                                <div class="flex justify-end gap-3 mt-5">
                                    <button type="button" @click="openInvite = false" class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition">{{ __('blog-invitation.modal_btn_cancel') }}</button>
                                    <button type="submit" :disabled="sending" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg transition disabled:opacity-50">
                                        <span x-show="!sending">{{ __('blog-invitation.modal_btn_send') }}</span>
                                        <span x-show="sending" x-cloak>{{ __('blog-invitation.modal_sending') }}</span>
                                    </button>
                                </div>
                            </form>
                        </div>
