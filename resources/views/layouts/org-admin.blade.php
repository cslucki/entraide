                ...
                <nav @click="if ($event.target.closest('a')) { pinned || (sidebarOpen = false) }" class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                    ...
                    @php
                        ...
                        $relationsItems = [
                            ...
                            // TASK-1450 : les ateliers (Workshop domain foundation).
                            ['route' => 'organization.admin.workshops', 'label' => __('navigation.org_admin_workshops'), 'icon' => 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253'],
                        ];
                        $adminItems = [
                        ...
                        // TASK-1305 : ordre = parcours naturel (etat -> configurer -> comportement
                        // -> connaissances -> consommation). ai-supervision / member-ai-profiles /
                        // ai-interactions restent des comingSoon() : masquees du menu tant qu'elles
                        // ne sont pas reelles — routes et controllers conserves, uniquement retires
                        // de la navigation. Identique pour toute Organization, `main` incluse :
                        // aucune branche `if main`.
                        $orgIaItems = [
                            // TASK-1223 : hub d'etat — le point d'entree.
                            ['route' => 'organization.admin.ai-cockpit', 'label' => __('navigation.org_admin_ai_cockpit'), 'icon' => 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z'],
                            // TASK-1212 / TASK-1305 : configuration du credential — deplacee
                            // depuis le groupe Administration, elle vit desormais dans le
                            // parcours IA lui-meme, juste apres le hub d'etat.
                    ...
                    <!-- Exchanges -->
                    @php $exchangesGroupActive = $isGroupActive($exchangesItems); @endphp
                    <div x-data="{ open: {{ $exchangesGroupActive ? 'true' : "localStorage.getItem('org_admin_sidebar_exchanges_open') !== 'false'" }} }">
                        <button @click.stop="open = !open; localStorage.setItem('org_admin_sidebar_exchanges_open', open)"
                            ...
                            <span class="text-xs font-semibold uppercase tracking-wider">{{ __('navigation.org_admin_section_exchanges') }}</span>
                        </button>
                        <div x-show="open" x-cloak
                             ...
                             class="origin-top">
                            @foreach($exchangesItems as $item)
                    ...
                    <!-- Content -->
                    @php $contentGroupActive = $isGroupActive($contentItems); @endphp
                    <div x-data="{ open: {{ $contentGroupActive ? 'true' : "localStorage.getItem('org_admin_sidebar_content_open') !== 'false'" }} }">
                        <button @click.stop="open = !open; localStorage.setItem('org_admin_sidebar_content_open', open)"
                            ...
                            <span class="text-xs font-semibold uppercase tracking-wider">{{ __('navigation.org_admin_section_content') }}</span>
                        </button>
                        <div x-show="open" x-cloak
                             x-transition:enter="transition ease-out duration-200"
                             x-transition:enter-start="opacity-0 scale-y-95"
                    ...
                    <!-- Community -->
                    @php $communityGroupActive = $isGroupActive($communityItems); @endphp
                    <div x-data="{ open: {{ $communityGroupActive ? 'true' : "localStorage.getItem('org_admin_sidebar_community_open') !== 'false'" }} }">
                        <button @click.stop="open = !open; localStorage.setItem('org_admin_sidebar_community_open', open)"
                            ...
                            <span class="text-xs font-semibold uppercase tracking-wider">{{ __('navigation.org_admin_section_community') }}</span>
                        </button>
                        <div x-show="open" x-cloak
                             x-transition:enter="transition ease-out duration-200"
                             x-transition:enter-start="opacity-0 scale-y-95"
                    ...
                    <!-- Administration -->
                    @php $adminGroupActive = $isGroupActive($adminItems); @endphp
                    <div x-data="{ open: {{ $adminGroupActive ? 'true' : "localStorage.getItem('org_admin_sidebar_admin_open') !== 'false'" }} }">
                        <button @click.stop="open = !open; localStorage.setItem('org_admin_sidebar_admin_open', open)"
                            ...
                            <span class="text-xs font-semibold uppercase tracking-wider">{{ __('navigation.org_admin_section_administration') }}</span>
                        </button>
                        <div x-show="open" x-cloak
                             x-transition:enter="transition ease-out duration-200"
                             x-transition:enter-start="opacity-0 scale-y-95"
                    ...
                    <!-- Design / Homepage + Themes -->
                    @php
                        $designItems = [
                            ['route' => 'organization.admin.homepage', 'label' => __('navigation.org_admin_homepage'), 'icon' => 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6'],
                            ['route' => 'organization.admin.themes', 'label' => __('navigation.org_admin_themes'), 'icon' => 'M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2v-4a2 2 0 00-2-2h-2.343M11 7.343l1.657-1.657a2 2 0 012.828 0l2.829 2.829a2 2 0 010 2.828L10.828 18.83'],
                        ];
                        $designGroupActive = $isGroupActive($designItems);
                    @endphp
                    <div x-data="{ open: {{ $designGroupActive ? 'true' : "localStorage.getItem('org_admin_sidebar_design_open') !== 'false'" }} }">
                        <button @click.stop="open = !open; localStorage.setItem('org_admin_sidebar_design_open', open)"
                            ...
                            <span class="text-xs font-semibold uppercase tracking-wider">{{ __('navigation.org_admin_design') }}</span>
                        </button>
                        <div x-show="open" x-cloak
                             x-transition:enter="transition ease-out duration-200"
                             x-transition:enter-start="opacity-0 scale-y-95"
                    ...
                    <!-- IA -->
                    @php $iaGroupActive = $isGroupActive($orgIaItems); @endphp
                    <div x-data="{ open: {{ $iaGroupActive ? 'true' : "localStorage.getItem('org_admin_sidebar_ia_open') !== 'false'" }} }">
                        <button @click.stop="open = !open; localStorage.setItem('org_admin_sidebar_ia_open', open)"
                            ...
                            <span class="text-xs font-semibold uppercase tracking-wider">{{ __('navigation.org_admin_ia') }}</span>
                        </button>
                        <div x-show="open" x-cloak
                             x-transition:enter="transition ease-out duration-200"
                             x-transition:enter-start="opacity-0 scale-y-95"
