<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto px-4 py-8">
        <div class="mb-8">
            ...
            <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">{{ $loop->name }}</p>
        </div>

        <form method="POST" action="{{ $_loopRoute('update', ['loop' => $loop]) }}" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-6">
            @csrf
            @method('PUT')

            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nom de la boucle *</label>
                <input type="text" name="name" id="name" value="{{ old('name', $loop->name) }}" required
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4"
                          class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-indigo-500 focus:border-transparent">{{ old('description', $loop->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ $_loopRoute('show', ['loop' => $loop]) }}"
