{{--
    Card « Membres » du workspace.

    Disposition bento : des tuiles autonomes, chacune une seule idee, posees sur
    le fond du panneau. La Card empilait auparavant un bandeau de comptage
    redondant avec le bouton du bandeau, trois sections dont les vides
    affichaient un tiret, puis deux formulaires pleine largeur — de quoi faire
    defiler pour trouver quatre noms.

    Les chiffres tiennent dans une rangee de tuiles, la liste respire, et les
    deux gestes d'ajout se rangent cote a cote des que la largeur le permet.
--}}
@php
    $roleRegistry = app(\App\Support\Loops\LoopRoleRegistry::class);
    $byRole = $loopMembers->groupBy(fn ($m) => $roleRegistry->canonical($m->role));
    $ownersCount = $byRole->get(\App\Support\Loops\LoopRoleRegistry::OWNER, collect())->count();
    $facilitatorsCount = $byRole->get(\App\Support\Loops\LoopRoleRegistry::FACILITATOR, collect())->count();
    $requestsCount = ($canManageJoinRequests ?? false) ? $pendingJoinRequests->count() : 0;

    $tile = 'rounded-2xl border border-[var(--bp-border)] bg-[var(--bp-panel)]';
@endphp

<div class="space-y-3">

    {{-- ── Les chiffres, en tuiles ────────────────────────────────────── --}}
    <div class="grid grid-cols-3 gap-2">
        @foreach([
            ['value' => $loopMembers->count(), 'label' => __('loops.governance_members'), 'tone' => 'text-[var(--bp-text)]'],
            ['value' => $ownersCount, 'label' => __('loops.governance_owners'), 'tone' => 'text-violet-600 dark:text-violet-300'],
            ['value' => $facilitatorsCount, 'label' => __('loops.governance_facilitators'), 'tone' => 'text-sky-600 dark:text-sky-300'],
        ] as $stat)
            <div class="{{ $tile }} px-3 py-2.5">
                <p class="text-xl font-bold leading-none tabular-nums {{ $stat['tone'] }}">{{ $stat['value'] }}</p>
                <p class="mt-1 truncate text-[10px] font-semibold uppercase tracking-wide text-[var(--bp-muted)]">{{ $stat['label'] }}</p>
            </div>
        @endforeach
    </div>

    {{-- ── Les demandes en attente ────────────────────────────────────── --}}
    @if($requestsCount > 0)
        <div class="rounded-2xl border border-amber-200 bg-amber-50 p-3 dark:border-amber-800/60 dark:bg-amber-900/15">
            <p class="mb-2 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wide text-amber-700 dark:text-amber-300">
                {{ __('loops.join_requests_title') }}
                <span class="inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-amber-500 px-1 text-[10px] font-bold text-white">{{ $requestsCount }}</span>
            </p>
            <ul class="space-y-2">
                @foreach($pendingJoinRequests as $joinRequest)
                    @php $rUser = $joinRequest->user; @endphp
                    <li class="rounded-xl border border-amber-200 bg-white p-2.5 dark:border-amber-800/50 dark:bg-gray-900">
                        <div class="flex items-center gap-2">
                            <x-user-avatar :user="$rUser" size="sm" />
                            <span class="min-w-0 flex-1 truncate text-sm font-semibold text-gray-800 dark:text-gray-100">{{ $rUser?->publicDisplayName() ?? '—' }}</span>
                            <span class="shrink-0 text-[10px] text-gray-400 dark:text-gray-500">{{ $joinRequest->created_at->diffForHumans() }}</span>
                        </div>
                        @if($joinRequest->message)
                            <p class="mt-1.5 text-xs leading-5 text-gray-600 dark:text-gray-300">{{ $joinRequest->message }}</p>
                        @endif
                        <div class="mt-2 flex gap-2">
                            <form method="POST" action="{{ route('loop-join-requests.accept', $joinRequest) }}" class="flex-1">
                                @csrf
                                <button type="submit" class="w-full rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-emerald-700">
                                    {{ __('loops.join_requests_accept') }}
                                </button>
                            </form>
                            <form method="POST" action="{{ route('loop-join-requests.reject', $joinRequest) }}" class="flex-1">
                                @csrf
                                <button type="submit" class="w-full rounded-lg border border-gray-300 px-3 py-1.5 text-xs font-semibold text-gray-600 transition hover:bg-gray-100 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-800">
                                    {{ __('loops.join_requests_reject') }}
                                </button>
                            </form>
                        </div>
                    </li>
                @endforeach
            </ul>
        </div>
    @endif

    {{-- ── Le trombinoscope ───────────────────────────────────────────── --}}
    <div class="{{ $tile }} p-3">
        @if($loopMembers->isEmpty())
            <p class="py-4 text-center text-sm text-[var(--bp-muted)]">{{ __($card['empty_title_key']) }}</p>
        @else
            <x-loops.governance-roster
                :members="$loopMembers"
                :role-route="fn($m) => route('loops.members.role', $m)"
                :remove-route="null"
                :can-manage-owners="$governance['owners'] ?? false"
                :can-manage-facilitators="$governance['facilitators'] ?? false"
                :can-remove="false"
                :creator-id="$currentLoop->created_by"
                :current-user-id="auth()->id()" />
        @endif
    </div>

    {{-- ── Les deux façons de faire entrer quelqu'un ──────────────────── --}}
    @unless($currentLoop->isArchived())
        <div class="grid gap-3 lg:grid-cols-2">

            {{-- Le cas courant : la personne a deja un compte ici. --}}
            @livewire('loop-member-picker', ['loop' => $currentLoop, 'showMembers' => false], key('member-picker-'.$currentLoop->id))

            {{-- L'autre cas : elle n'est pas encore sur BouclePro. Cible sur
                 CETTE Boucle (TASK-1077) — le formulaire generique d'avant
                 envoyait un lien d'inscription qui ne rejoignait rien. --}}
            <div class="{{ $tile }} p-4">
                <p class="flex items-center gap-2 text-sm font-semibold text-[var(--bp-text)]">
                    <svg class="h-4 w-4 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75"/></svg>
                    {{ __('loops.members_invite_title') }}
                </p>

                <form method="POST" action="{{ $loopInvitationAction }}" class="mt-3 space-y-2">
                    @csrf
                    <input type="email" name="email" required placeholder="{{ __('loops.members_invite_email_placeholder') }}"
                           class="w-full rounded-xl border-[var(--bp-border)] bg-[var(--bp-surface)] text-sm text-[var(--bp-text)] focus:border-[var(--bp-primary)] focus:ring-[var(--bp-primary)]">
                    <input type="text" name="name" maxlength="255" placeholder="{{ __('loops.members_invite_name_placeholder') }}"
                           class="w-full rounded-xl border-[var(--bp-border)] bg-[var(--bp-surface)] text-sm text-[var(--bp-text)] focus:border-[var(--bp-primary)] focus:ring-[var(--bp-primary)]">
                    <button type="submit" class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[var(--bp-primary)] px-4 py-2.5 text-sm font-semibold text-white transition hover:opacity-90">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 12 3.269 3.125A59.769 59.769 0 0 1 21.485 12 59.768 59.768 0 0 1 3.27 20.875L5.999 12Zm0 0h7.5"/></svg>
                        {{ __('loops.members_invite_submit') }}
                    </button>
                </form>

                <x-loops.invitation-list :invitations="$loopInvitations ?? collect()" class="mt-3 border-t border-[var(--bp-border)] pt-3" />
            </div>
        </div>
    @endunless
</div>
