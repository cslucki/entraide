    ...
    <section class="relative overflow-hidden" style="background: linear-gradient(135deg, {{ $organization->accent_color }} 0%, {{ $organization->accent_color }}dd 100%);">
        ...
        <div class="relative max-w-4xl mx-auto px-4 py-24 sm:py-32 text-center">
            <h1 class="text-4xl sm:text-5xl font-bold text-white mb-4">
                {{ $organization->hero_title ?: $organization->name }}
            </h1>
            @if($organization->hero_description)
            <p class="text-xl text-white/90 max-w-2xl mx-auto mb-8">{{ $organization->hero_description }}</p>
            @endif
            <div class="flex flex-wrap justify-center gap-3">
                @auth
                    <a href="{{ route('organization.dashboard', ['organization' => $organization->slug]) }}"
                       class="px-6 py-3 bg-white text-gray-900 font-semibold rounded-lg hover:bg-gray-100 transition shadow-lg">
                        Accéder au tableau de bord
                    </a>
                    <a href="{{ route('organization.explorer', ['organization' => $organization->slug]) }}"
                ...
                @else
                    <a href="{{ route('organization.register', ['organization' => $organization->slug]) }}"
                       class="px-6 py-3 bg-white text-gray-900 font-semibold rounded-lg hover:bg-gray-100 transition shadow-lg">
                        Rejoindre l'organisation
                    </a>
                    <a href="{{ route('organization.login', ['organization' => $organization->slug]) }}"
                       class="px-6 py-3 bg-white/20 text-white font-semibold rounded-lg hover:bg-white/30 transition shadow-lg border border-white/30">
                        Se connecter
                    </a>
                @endauth
    ...
    <section class="max-w-6xl mx-auto px-4 py-12">
        ...
        @if($recentServices->isEmpty())
            <div class="text-center py-12 text-gray-500 dark:text-gray-400">
                ...
                <a href="{{ route('organization.register', ['organization' => $organization->slug]) }}" class="text-indigo-600 hover:text-indigo-800 mt-2 inline-block">
                    Rejoindre l'organisation pour publier votre premier service
                </a>
            </div>
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($recentServices as $service)
                <a href="{{ route('organization.services.show', ['organization' => $organization->slug, 'service' => $service]) }}"
    ...
    <section class="bg-gray-50 dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700">
        <div class="max-w-4xl mx-auto px-4 py-8 text-center">
            <p class="text-sm text-gray-500 dark:text-gray-400">
                {{ $organization->type_label ?? 'Organisation' }} <strong class="text-gray-700 dark:text-gray-300">{{ $organization->name }}</strong> — Propulsée par BouclePro
            </p>
        </div>
    </section>
</div>
