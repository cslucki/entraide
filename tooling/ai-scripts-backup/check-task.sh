...
# =========================================================
# CHECK TASK SCRIPT — WORKTREE-LOCAL COPY (TASK-1200)
# =========================================================
# Adapte depuis test.laravel/ai/scripts/check-task.sh.
...
CODE_BASE_DIR="${CHECK_CODE_BASE_DIR:-$BASE_DIR}"
...
# validation_level (TASK-1150) : declare, pas devine. Enum courte,
# volontairement sans moteur de scoring.
VALID_LEVELS = {'MICRO_UX', 'STANDARD', 'SENSITIVE', 'NO_APP_CI'}

print(f"Validation level: {validation_level or '(none)'}")

level_ok = validation_level in VALID_LEVELS
if not level_ok:
    detail = f"validation_level must be one of {sorted(VALID_LEVELS)} (found: {validation_level or 'unset'})."
    if dry_run:
        print(f"  WARNING: {detail}")
    else:
        print(f"  ERROR: {detail}")

if status != 'DONE' or lock_status != 'UNLOCKED' or (not level_ok and not dry_run):
    sys.exit(1)
PYEOF
...
# =========================================================
# 6. VERSION CHECK — validates project version format (warning in dry-run)
# =========================================================

if [ "$CURRENT_BRANCH" != "main" ] && [ "$CURRENT_BRANCH" != "develop" ]; then
  VERSION_FILE="$CODE_BASE_DIR/VERSION"
  if [ -f "$VERSION_FILE" ]; then
    ACTUAL_VERSION=$(cat "$VERSION_FILE" | tr -d '[:space:]')
    echo "Version: $ACTUAL_VERSION"
    if [[ ! $ACTUAL_VERSION =~ ^[0-9]+\.[0-9]{3}$ ]]; then
      if [ "$DRY_RUN" = true ]; then
        echo "  WARNING: Expected version format MAJOR.PATCH with 3-digit patch (example: 1.000)."
        echo "  Run 'bump-version.sh' on this branch before finalizing."
      else
        echo "  ERROR: Expected version format MAJOR.PATCH with 3-digit patch (example: 1.000)."
        echo "  Run 'bump-version.sh' on this branch before finalizing."
        RESULT=$FAIL
      fi
    else
      echo "  Version format OK (independent of task number)."
    fi
    echo ""
  fi
fi
