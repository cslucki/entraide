...
class UserDataLifecycleRegistry
{
    ...
    public static function entries(): array
    {
        return [
            ...
            ['key' => 'organization_loop_plugins_updated_by', 'type' => 'sql', 'table' => 'organization_loop_plugins', 'column' => 'updated_by', 'policy' => self::POLICY_DETACH, 'org_scope' => 'direct', 'justification' => 'Whether a Loop plugin is allowed in an Organization is a super-admin administration decision, not personal data: it must outlive its author, who is simply detached. The FK is nullOnDelete, so the decision survives anonymously (TASK-1614).'],
            ['key' => 'loop_plugins_updated_by', 'type' => 'sql', 'table' => 'loop_plugins', 'column' => 'updated_by', 'policy' => self::POLICY_DETACH, 'org_scope' => 'direct', 'justification' => 'Whether a plugin is enabled in a Loop is Loop configuration, not personal data: it must outlive whoever switched it on, who is simply detached. The FK is nullOnDelete (TASK-1616).'],
            ...
            ['key' => 'guest_visitors_claimed', 'type' => 'sql', 'table' => 'guest_visitors', 'column' => 'claimed_user_id', 'policy' => self::POLICY_DETACH, 'org_scope' => 'direct', 'justification' => 'Guest visitor claim link is detached; the pseudonymous visitor record keeps its own retention.'],
            ['key' => 'guest_conversations_claimed', 'type' => 'sql', 'table' => 'guest_conversations', 'column' => 'claimed_user_id', 'policy' => self::POLICY_DETACH, 'org_scope' => 'direct', 'justification' => 'Guest conversation claim link is detached; the conversation follows its visitor retention.'],
            // TASK-1439 — UsageReference V1 : texte editorial PLATEFORME (aucune Organization) ; l'attribution auteur/publieur est un audit retenu, la FK passe a NULL si le compte disparait.
            ['key' => 'usage_references_created_by', 'type' => 'sql', 'table' => 'usage_references', 'column' => 'created_by', 'policy' => self::POLICY_RETAIN, 'org_scope' => 'none', 'justification' => 'Platform usage reference editorial audit is retained; the author FK nulls on user deletion.'],
            ['key' => 'usage_references_published_by', 'type' => 'sql', 'table' => 'usage_references', 'column' => 'published_by', 'policy' => self::POLICY_RETAIN, 'org_scope' => 'none', 'justification' => 'Platform usage reference publication audit is retained; the publisher FK nulls on user deletion.'],
            // TASK-1446 — AcquisitionJourney : definition tenantee d'un parcours ; l'attribution auteur/publieur est un audit retenu, FK nullOnDelete.
            ['key' => 'acquisition_journeys_created_by', 'type' => 'sql', 'table' => 'acquisition_journeys', 'column' => 'created_by', 'policy' => self::POLICY_RETAIN, 'org_scope' => 'direct', 'justification' => 'Acquisition journey editorial audit is retained; the author FK nulls on user deletion.'],
            ['key' => 'acquisition_journeys_published_by', 'type' => 'sql', 'table' => 'acquisition_journeys', 'column' => 'published_by', 'policy' => self::POLICY_RETAIN, 'org_scope' => 'direct', 'justification' => 'Acquisition journey publication audit is retained; the publisher FK nulls on user deletion.'],
            // TASK-1447 — OrganizationShortcut : audit de creation plateforme, FK nullOnDelete.
            ['key' => 'organization_shortcuts_created_by', 'type' => 'sql', 'table' => 'organization_shortcuts', 'column' => 'created_by', 'policy' => self::POLICY_RETAIN, 'org_scope' => 'direct', 'justification' => 'Shortcut creation audit is retained; the author FK nulls on user deletion.'],
            // TASK-1449 : journal d'acquisition append-only (faits de parcours, aucune donnee personnelle) — le fait survit au compte, la FK devient nulle.
            ['key' => 'acquisition_events_user_id', 'type' => 'sql', 'table' => 'acquisition_events', 'column' => 'user_id', 'policy' => self::POLICY_RETAIN, 'org_scope' => 'direct', 'justification' => 'Acquisition funnel facts are retained as pseudonymous tenant analytics; the user FK nulls on user deletion.'],
            // TASK-1450 : ateliers (Workshop domain foundation) — audit editorial retenu, FK auteur/publication nulles a la suppression.
            ['key' => 'workshops_created_by', 'type' => 'sql', 'table' => 'workshops', 'column' => 'created_by', 'policy' => self::POLICY_RETAIN, 'org_scope' => 'direct', 'justification' => 'Workshop editorial audit is retained; the author FK nulls on user deletion.'],
            ['key' => 'workshops_published_by', 'type' => 'sql', 'table' => 'workshops', 'column' => 'published_by', 'policy' => self::POLICY_RETAIN, 'org_scope' => 'direct', 'justification' => 'Workshop publication audit is retained; the publisher FK nulls on user deletion.'],
            // TASK-1451 (B4-A) : sessions d'atelier — audit editorial retenu, FK auteur nulle a la suppression.
            ['key' => 'workshop_sessions_created_by', 'type' => 'sql', 'table' => 'workshop_sessions', 'column' => 'created_by', 'policy' => self::POLICY_RETAIN, 'org_scope' => 'direct', 'justification' => 'Workshop session editorial audit is retained; the author FK nulls on user deletion.'],
            // TASK-1453 : inscriptions aux sessions — la participation d'un compte supprime disparait avec lui (cascade).
            ['key' => 'workshop_registrations_user_id', 'type' => 'sql', 'table' => 'workshop_registrations', 'column' => 'user_id', 'policy' => self::POLICY_DELETE, 'org_scope' => 'direct', 'justification' => 'A workshop registration is personal participation data; it is deleted with the user (FK cascade).'],
